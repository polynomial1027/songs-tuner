...
- 自由练声：麦克风持续监听，自动判断最近标准音及 cents 偏差，也可锁定一个目标音
- 导入 `.singright.json` 曲谱
- 左侧曲目库可删除任意导入、新建或内置曲谱，删除结果保存在本机
- 上传 WAV、MP3、M4A 等浏览器可解码录音并离线分析
- 独立练习速度、半音升降调、精确首音定调、A4 参考频率与容差设置
- 内置五线谱与简谱工作台：Enter 开启键盘录入，1–5 选时值，Q W E R T Y U I O P [ ] 输入十二音，Space 保留为当前时值休止符
- 以十六分音符为最小固定网格；音符在占用时段内居中，小节末自动续写，方向键控制光标/八度，Backspace 删除前一个音
- 支持同音延音线及合并时值、高低音谱号切换、时间轴调号/还原、临时升降记号和反复线
- 所有制谱快捷键可自定义；Shift+音键临时升半音，Ctrl+音键临时降半音
- 80%–200% 全界面缩放，练习室与制谱器同步适配高分辨率屏幕
- 长曲谱保持固定间距，支持自动跟随光标/播放头、按住拖动查看和自动回到当前位置
- 100 步撤销/重做、自动保存、复制、删除和精确拍点编辑
- 制谱试听与逐音、连续跟唱、整曲复盘均提供可独立开关的节拍器
- 练习界面可试听当前目标音或按当前速度、移调和定调结果试听整首曲谱
- 合成音试听、预备拍、小节区间循环与键盘快速循环当前小节
- 参考音频波形、裁剪、偏移、音量和播放速度调整
- 曲谱、模板和录音使用系统原生“另存为”窗口；支持 MusicXML 导入导出与标准 MIDI 导出
- 首次安装只放置一个低到高音阶示例；空白模板仍可下载或从制谱器新建
