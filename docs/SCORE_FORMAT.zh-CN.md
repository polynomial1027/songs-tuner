...
- `notation.keyChanges[]`：可选，按 `beat` 保存曲中调号变化；`fifths: 0` 表示还原调号，并持续到下一次变化。
- `notation.clefChanges[]`：可选，按 `beat` 保存高音/低音谱号变化。
- `notation.repeats[]`：可选，保存 `start` / `end` 反复线所在的小节边界。
...
- `notes[].explicitAccidental`：可选，临时 `flat`、`natural` 或 `sharp` 记号。
- `notes[].tieToNext`：可选；为 `true` 时连接紧邻、相同音高的下一音，练唱、试听和 MIDI 导出按合并时值处理。
...
- 所有槽位都以十六分音符为最小单位；更长音符会占用连续多个格子。音符在占用时段内居中，小节末自动进入下一小节。
- `Space` 固定输入当前时值的标准休止符；十六分音符不提供需要三十二分网格的附点。
- `H` 添加/移除同音延音线，`J` 切换谱号，`L`/`;` 添加反复开始/结束线，`K` 循环当前小节。
- `Z`/`X`/`C` 在光标处减少调号、还原调号或增加调号。调号持续生效到下一次变化；`Shift`+音键临时升半音，`Ctrl`+音键临时降半音，`N` 写入还原记号。
- 除固定保留的 Space 休止符外，制谱快捷键均可在设置中重新绑定。
- 音符不能重叠或越过小节线；每小节容量由拍号严格确定。
- 设置提供 80%–200% 全界面缩放，用于高分辨率屏幕，练习室和制谱器共用同一设置。
- 长曲谱不会继续压缩音符：制谱器和练习时间轴会自动跟随光标或播放头，也可以按住谱面拖动；松开后会自动回到当前编辑/练习位置。
- 制谱试听、逐音校准、连续跟唱和整曲复盘都有独立节拍器开关。
