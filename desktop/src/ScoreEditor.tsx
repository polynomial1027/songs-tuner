...
function StaffCanvas({
  ...
  return (
    <div className="notation-scroll" aria-label="可编辑五线谱">
      <svg
        className={`notation-svg tool-${tool}`}
        viewBox={`0 0 ${SVG_WIDTH} ${height}`}
        role="application"
        aria-label="点击谱面输入音符；点击已有音符进行编辑"
      >
              ...
              {score.notes.filter((note) => note.beat >= systemStart && note.beat < systemStart + beatsPerSystem).map((note) => {
                ...
                return (
                  <g
                    ...
                    key={note.id}
                    role="button"
                    aria-label={`${note.midi === null ? "休止符" : midiToNoteName(note.midi)}，${durationName(note.durationBeats)}，第 ${note.beat + 1} 拍`}
                    onClick={(event) => {
                  ...
                  >
                    ...
                    {note.midi === null ? (
                      <text className="rest-glyph" x={x} y={noteY + 7}>休</text>
                    ) : (
                      <>
...
export default function ScoreEditor({ initialScore, onCommit, onClose }: ScoreEditorProps) {
  ...
  }, [accidental, addNote, commit, draft.notes, inputOctave, isPlaying, redo, selectedNote, startPlayback, stopPlayback, undo]);
  ...
  async function attachAudio(file: File | undefined) {
    ...
    try {
      ...
      setToast("参考音频已加载，可在波形上定位");
    } catch {
      setToast("无法读取这份音频，请尝试 WAV、MP3、M4A 或 WebM");
    } finally {
  ...
  function save(close: boolean) {
    try {
      ...
      setSavedAt(Date.now());
      setToast(close ? "曲谱已保存，正在返回练习室" : "曲谱已保存到曲目列表");
    } catch (error) {
      setToast(error instanceof Error ? error.message : "曲谱校验失败");
    }
  }
    ...
    <div className="composer-shell">
      <header className="composer-topbar">
        <div className="composer-document">
          <button className="icon-button back" onClick={onClose} aria-label="返回练习室"><ArrowLeft /></button>
          <div className="composer-brand"><span><ListMusic /></span><div><small>SINGRIGHT COMPOSER</small><strong>五线谱工作台</strong></div></div>
          <i />
          <div className="document-status">
            <strong>{draft.metadata.title || "未命名曲谱"}</strong>
            <span>{savedAt ? `草稿已自动保存 · ${new Date(savedAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}` : "正在准备自动保存"}</span>
          </div>
        </div>
        <div className="composer-actions">
          <button className="icon-button" disabled={!past.length} onClick={undo} title="撤销（⌘Z）"><Undo2 /></button>
          <button className="icon-button" disabled={!future.length} onClick={redo} title="重做（⇧⌘Z）"><Redo2 /></button>
          <button className="text-button" onClick={recoverAutosave}><RotateCcw /> 恢复草稿</button>
          <button className="text-button" onClick={() => notationInputRef.current?.click()}><FileInput /> 导入</button>
          ...
          <div className="export-menu">
            <button className="text-button"><Download /> 导出</button>
            <div><button onClick={() => exportScore("json")}>SingRight JSON<small>继续编辑与练唱</small></button><button onClick={() => exportScore("musicxml")}>MusicXML<small>MuseScore / Sibelius</small></button><button onClick={() => exportScore("midi")}>标准 MIDI<small>用于编曲软件</small></button></div>
          </div>
          <button className="save-button" onClick={() => save(false)}><Save /> 保存</button>
          <button className="practice-button" onClick={() => save(true)}><Headphones /> 保存并练习</button>
        </div>
      </header>
      ...
      <div className="composer-body">
        <aside className="composer-toolbox">
          <div className="tool-section">
            <span className="tool-section-title">输入工具</span>
            <div className="tool-mode-grid">
              <button className={tool === "select" ? "active" : ""} onClick={() => setTool("select")}><MousePointer2 /><span>选择</span><small>移动光标</small></button>
              <button className={tool === "note" ? "active" : ""} onClick={() => setTool("note")}><Music /><span>音符</span><small>点击谱线</small></button>
              <button className={tool === "rest" ? "active" : ""} onClick={() => setTool("rest")}><span className="rest-tool">休</span><span>休止</span><small>填充空拍</small></button>
            </div>
          </div>

          <div className="tool-section">
            <div className="tool-section-title"><span>音符时值</span><small>快捷键 1–5</small></div>
            <div className="duration-grid">
              {DURATION_OPTIONS.map((option) => (
                ...
                ><strong>{option.glyph}</strong><span>{option.label.replace("音符", "")}</span><kbd>{option.shortcut}</kbd></button>
              ))}
            </div>
            <button className={`dot-toggle ${dotted ? "active" : ""}`} onClick={() => setDotted((value) => !value)}>
              <i /> 附点 <span>{dotted ? `${effectiveDuration} 拍` : "延长一半"}</span>
            </button>
          </div>

          <div className="tool-section">
            <span className="tool-section-title">升降记号</span>
            <div className="accidental-grid">
              {([
                { value: -1, glyph: "♭", label: "降" },
                { value: 0, glyph: "♮", label: "还原" },
                { value: 1, glyph: "♯", label: "升" },
              ] as const).map((item) => (
          ...
          <div className="tool-section piano-section">
            <div className="tool-section-title"><span>键盘输入</span><small>A–G 录入</small></div>
            <div className="octave-stepper">
              <button onClick={() => setInputOctave((value) => Math.max(1, value - 1))}><Minus /></button>
              <span>第 <strong>{inputOctave}</strong> 组</span>
              ...
              {PITCH_CLASSES.map((key) => {
                ...
                return (
                  <button
                    ...
                    aria-label={`输入 ${key.label}${inputOctave}`}
                  >{key.label}</button>
                );
              })}
            </div>
          ...
          <div className="shortcut-note">
            <Keyboard />
            <div><strong>高效录入</strong><span>A–G 输入音符 · R 输入休止<br />方向键选音/移调 · 空格试听</span></div>
          </div>
        </aside>

        <main className="composer-workspace">
          <section className="score-setup-bar">
            <label><span>速度</span><div><input type="number" min="20" max="300" value={draft.tempo.bpm} onChange={(event) => commit((current) => ({ ...current, tempo: { bpm: Math.max(20, Math.min(300, Number(event.target.value))) } }))} /><small>BPM</small></div></label>
            <label><span>拍号</span><div><select value={draft.timeSignature.beats} onChange={(event) => commit((current) => ({ ...current, timeSignature: { ...current.timeSignature, beats: Number(event.target.value) } }))}>{[2, 3, 4, 5, 6, 7, 9, 12].map((value) => <option key={value}>{value}</option>)}</select><b>/</b><select value={draft.timeSignature.beatUnit} onChange={(event) => commit((current) => ({ ...current, timeSignature: { ...current.timeSignature, beatUnit: Number(event.target.value) as 1 | 2 | 4 | 8 | 16 } }))}>{[2, 4, 8, 16].map((value) => <option key={value}>{value}</option>)}</select></div></label>
            <label className="key-field"><span>调号</span><select value={draft.notation?.keySignature ?? 0} onChange={(event) => commit((current) => ({ ...current, notation: { clef: current.notation?.clef ?? "treble", keySignature: Number(event.target.value) } }))}>{KEY_SIGNATURES.map((key) => <option key={key.fifths} value={key.fifths}>{key.name}</option>)}</select></label>
            <label><span>谱号</span><select value={draft.notation?.clef ?? "treble"} onChange={(event) => commit((current) => ({ ...current, notation: { keySignature: current.notation?.keySignature ?? 0, clef: event.target.value as Clef } }))}><option value="treble">𝄞 高音谱号</option><option value="bass">𝄢 低音谱号</option></select></label>
            <div className="setup-summary"><Grid2X2 /><span><strong>{measureCount}</strong> 小节</span><span><strong>{draft.notes.length}</strong> 音符</span></div>
          </section>

          <section className="score-paper">
            <div className="paper-toolbar">
              <div className="editor-transport">
                <button className="jump-start" onClick={() => { stopPlayback(false); setPlaybackBeat(0); }} title="回到开头">|‹</button>
                <button className={`editor-play ${isPlaying ? "playing" : ""}`} onClick={() => isPlaying ? stopPlayback(false) : void startPlayback()}>
                  {isPlaying ? <Pause fill="currentColor" /> : <Play fill="currentColor" />}
                  {countdown ? `${countdown}…` : isPlaying ? "暂停试听" : "试听谱面"}
                </button>
                <button className="stop-playback" onClick={() => stopPlayback(true)}><CircleStop /> 停止</button>
              </div>
              <div className="play-options">
                <label><input type="checkbox" checked={countIn} onChange={(event) => setCountIn(event.target.checked)} /> 预备拍</label>
                <label><input type="checkbox" checked={metronome} onChange={(event) => setMetronome(event.target.checked)} /> 节拍器</label>
                <label><input type="checkbox" checked={loop} onChange={(event) => setLoop(event.target.checked)} /> 循环</label>
                {loop && <span className="loop-range">第 <input type="number" min="1" max={measureCount} value={loopStartMeasure} onChange={(event) => setLoopStartMeasure(Math.max(1, Number(event.target.value)))} /> 至 <input type="number" min={loopStartMeasure} max={measureCount + 1} value={loopEndMeasure} onChange={(event) => setLoopEndMeasure(Math.max(loopStartMeasure, Number(event.target.value)))} /> 小节</span>}
              </div>
              <div className="zoom-control"><span>谱面</span><button onClick={() => setZoom((value) => Math.max(1, value - 1))}><Minus /></button><strong>{Math.round((zoom / 3) * 100)}%</strong><button onClick={() => setZoom((value) => Math.min(5, value + 1))}><Plus /></button></div>
            </div>
            <div className="paper-title">
              <div><h1>{draft.metadata.title || "未命名曲谱"}</h1><p>{draft.metadata.artist || "词曲作者"}</p></div>
            ...
            />
            <div className="paper-footer"><span>单声部旋律谱 · 点击谱线输入，选中后用方向键微调</span><strong>第 {Math.floor(cursorBeat / draft.timeSignature.beats) + 1} 小节 · 第 {(cursorBeat % draft.timeSignature.beats) + 1} 拍</strong></div>
          </section>
          ...
          <section className={`audio-guide-panel ${audioPanelOpen ? "open" : ""}`}>
            <button className="audio-panel-heading" onClick={() => setAudioPanelOpen((value) => !value)}>
              <div><span className="audio-track-icon"><AudioWaveform /></span><span><strong>参考音频轨</strong><small>听原曲、看波形、对齐拍点后打谱</small></span></div>
              <i>{audioPanelOpen ? "收起" : "展开"}</i>
            </button>
            {audioPanelOpen && (
              <div className="audio-panel-body">
                {!audioUrl ? (
                  <div className="audio-drop">
                    <FileAudio2 />
                    <div><strong>{audioGuide ? `请重新选择：${audioGuide.name}` : "添加参考音频"}</strong><span>音频只在本机打开，不会写入曲谱或上传</span></div>
                    <button onClick={() => audioInputRef.current?.click()}>{audioGuide ? "重新关联文件" : "选择音频"}</button>
                  </div>
                ) : (
                  <>
                    <div className="wave-track-info">
                      ...
                      <span><strong>{audioGuide?.name}</strong><small>{Math.floor(audioDuration / 60)}:{String(Math.floor(audioDuration % 60)).padStart(2, "0")} · 本机音频</small></span>
                      <Volume2 />
                    </div>
                    <Waveform
                      values={waveform}
                    ...
                    <div className="audio-edit-controls">
                      <label><Scissors /><span>裁剪开始</span><input type="number" min="0" max={trimEnd} step="0.1" value={trimStart.toFixed(1)} onChange={(event) => commit((current) => ({ ...current, audioGuide: current.audioGuide ? { ...current.audioGuide, trimStartSeconds: Math.min(trimEnd, Number(event.target.value)) } : undefined }))} /><small>秒</small></label>
                      <label><Scissors /><span>裁剪结束</span><input type="number" min={trimStart} max={audioDuration} step="0.1" value={trimEnd.toFixed(1)} onChange={(event) => commit((current) => ({ ...current, audioGuide: current.audioGuide ? { ...current.audioGuide, trimEndSeconds: Math.max(trimStart, Number(event.target.value)) } : undefined }))} /><small>秒</small></label>
                      <label><Clock3 /><span>谱前偏移</span><input type="number" min="-30" max="30" step="0.1" value={audioGuide?.offsetSeconds ?? 0} onChange={(event) => commit((current) => ({ ...current, audioGuide: current.audioGuide ? { ...current.audioGuide, offsetSeconds: Number(event.target.value) } : undefined }))} /><small>秒</small></label>
                      <label><Volume2 /><span>音量</span><input type="range" min="0" max="1" step="0.05" value={audioGuide?.gain ?? 0.75} onChange={(event) => commit((current) => ({ ...current, audioGuide: current.audioGuide ? { ...current.audioGuide, gain: Number(event.target.value) } : undefined }))} /></label>
                      <label><Repeat2 /><span>速度</span><select value={audioGuide?.playbackRate ?? 1} onChange={(event) => commit((current) => ({ ...current, audioGuide: current.audioGuide ? { ...current.audioGuide, playbackRate: Number(event.target.value) } : undefined }))}><option value="0.5">0.5×</option><option value="0.75">0.75×</option><option value="1">1×</option><option value="1.25">1.25×</option><option value="1.5">1.5×</option></select></label>
                    </div>
                  </>
                )}
        ...
        <aside className="composer-inspector">
          <div className="inspector-head"><span>属性检查器</span><strong>{selectedNote ? "当前音符" : "曲谱信息"}</strong></div>
          <section className="document-fields">
            <label><span>曲名</span><input value={draft.metadata.title} onChange={(event) => commit((current) => ({ ...current, metadata: { ...current.metadata, title: event.target.value } }))} /></label>
            <label><span>词曲作者</span><input value={draft.metadata.artist ?? ""} placeholder="选填" onChange={(event) => commit((current) => ({ ...current, metadata: { ...current.metadata, artist: event.target.value } }))} /></label>
            <label><span>说明</span><textarea rows={2} value={draft.metadata.description ?? ""} placeholder="练习提示、歌曲段落等" onChange={(event) => commit((current) => ({ ...current, metadata: { ...current.metadata, description: event.target.value } }))} /></label>
          </section>

          {selectedNote ? (
            <section className="note-inspector">
              <div className="selected-note-card">
                <div className={selectedNote.midi === null ? "rest" : ""}>{selectedNote.midi === null ? "休" : "♩"}</div>
                <span><strong>{selectedNote.midi === null ? "休止符" : midiToNoteName(selectedNote.midi)}</strong><small>{durationName(selectedNote.durationBeats)} · 第 {selectedNote.beat + 1} 拍</small></span>
              </div>
              {selectedNote.midi !== null && (
                <label><span>音高</span><div className="stepper wide"><button onClick={() => {
              ...
              <label><span>起始拍</span><input type="number" min="0" step="0.25" value={selectedNote.beat} onChange={(event) => patchSelected({ beat: Math.max(0, Number(event.target.value)) })} /></label>
              <label><span>时值</span><select value={selectedNote.durationBeats} onChange={(event) => patchSelected({ durationBeats: Number(event.target.value) })}>{DURATION_OPTIONS.flatMap((option) => [<option key={option.beats} value={option.beats}>{option.label} · {option.beats} 拍</option>, <option key={`${option.beats}-dot`} value={option.beats * 1.5}>附点{option.label} · {option.beats * 1.5} 拍</option>])}</select></label>
              {selectedNote.midi !== null && <label><span>唱名</span><input value={selectedNote.numeral ?? ""} placeholder={numeralForMidi(selectedNote.midi, draft.tuning.tonicMidi)} onChange={(event) => patchSelected({ numeral: event.target.value })} /></label>}
              <label><span>歌词</span><input value={selectedNote.lyric ?? ""} placeholder="当前音对应的字" onChange={(event) => patchSelected({ lyric: event.target.value })} /></label>
              <div className="note-actions"><button onClick={duplicateSelected}><Copy /> 复制到下一拍</button><button className="danger" onClick={removeSelected}><Trash2 /> 删除</button></div>
            </section>
          ) : (
            <div className="inspector-empty"><MousePointer2 /><strong>选择一个音符</strong><span>点击谱面中的音符，在这里精确编辑音高、拍点、时值、唱名和歌词。</span></div>
          )}

          <section className="tuning-fields">
            <div className="inspector-section-title"><AlignLeft /><span><strong>练唱基准</strong><small>保存后直接用于音准判定</small></span></div>
            <label><span>简谱 1 =</span><select value={draft.tuning.tonicMidi} onChange={(event) => commit((current) => ({ ...current, tuning: { ...current.tuning, tonicMidi: Number(event.target.value) } }))}>{Array.from({ length: 36 }, (_, index) => 48 + index).map((midi) => <option key={midi} value={midi}>{midiToNoteName(midi)}</option>)}</select></label>
            <label><span>A4 基准</span><div><input type="number" min="430" max="450" value={draft.tuning.referenceHz} onChange={(event) => commit((current) => ({ ...current, tuning: { ...current.tuning, referenceHz: Number(event.target.value) } }))} /><small>Hz</small></div></label>
          </section>

          <div className="editor-help">
            <strong>不会写五线谱？</strong>
            <p>先选时值，再直接点击谱线或钢琴键。橙色光标会自动前进；输错时按 ⌘Z / Ctrl+Z。</p>
          </div>
        </aside>
      </div>
